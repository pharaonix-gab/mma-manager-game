...
function styleMatchupEdge(oppStyleId, tacticFocus){
  const beats = { lutteur:"boxeur", grappler:"lutteur", boxeur:"grappler" };
  ...
  if (beats[oppStyleId] === playerStyle) return 1;
  if (beats[playerStyle] === oppStyleId) return -1;
  return 0;
}
...
function scoreRoundForJudge(delta, judge){
  ...
  if (Math.abs(noisy) < 1.2 && Math.random() < 0.12) return [10,10];
...
// ---- Classement mondial dynamique : un top 15 vivant par organisation ----
...
function updatePlayerRankOnWin(ns, org, isTitle){
  ...
  if (isTitle){ ns.orgRanks[orgId] = 0; return ns; }
  const current = ns.orgRanks[orgId];
  if (current === 0) return ns;
...
// ---- Classement mondial (Top 50 pound-for-pound) ----
...
// ---- Sponsors avec objectifs ----
const SPONSOR_OBJECTIVES = [
  { id:"ko_wins", label:"Gagner par KO", check:(s,before)=> (s.koWins||0) > (before.koWins||0) },
  { id:"three_fights_year", label:"Combattre 3 fois cette année", check:(s,before)=> true },
...
function travelCostsFor(org, s){
  ...
  const staffCount = (s.hiredStaff||[]).length + 1;
  const nights = 5;
...
// ---- Incidents de conférence de presse ----
...
// ---- Moments décisifs en cours de combat ----
const MOMENTS = [
...
// ---- Techniques à découvrir + Arbre passif ----
const PASSIVE_PERKS = [
...
const TECHNIQUES = [
  ...
  { id:"ankle_pick", name:"Ankle pick", discipline:"lutte", tier:2, desc:"Projection rapide sur une cheville dégagée." },
...
function tryEarlyFinish(opt, tactic, s){
  ...
  const extremity = Math.abs(winChance-50)/50;
  ...
  if (opt.roundIndex >= 3){
    if (traits.includes("faibleCardio")) finishChance += 0.10;
    if (traits.includes("grosCardio")) finishChance -= 0.06;
  }
  if (traits.includes("finisseur")) finishChance += 0.05;
  if (traits.includes("patient") && opt.roundIndex <= 2) finishChance -= 0.07;
  if (winChance >= 50){
    if (traits.includes("bonDefenseur")) finishChance -= 0.06;
    if (traits.includes("mauvaisDefenseur")) finishChance += 0.08;
...
function pickDecisionMethod(opt){
  ...
  else if (playerCards===1 && oppCards===1 && drawCards===1){ decisionType = "partagée"; decisionWin = null; }
...
const EVENTS = [
  ...
  // ================= SUITE COMBAT / ENTRAÎNEMENT / BLESSURES =================
  ...
  // ================= COHÉRENCE AVEC L'ISSUE DU DERNIER COMBAT =================
...
export default function MMACareer(){
  ...
  function startCoachingCareer(){
    ...
    setCoachCareer({
      gymName, season: 1, prospects: [], fightersTrained: 0, championsProduced: 0, ufcSignings: 0,
      log: [`🏋️ ${name} raccroche les gants et ouvre ${gymName}, bien décidé(e) à former la nouvelle génération.`],
    });
    setPhase("coaching");
  }
  function advanceCoachingSeason(){
    setCoachCareer(cc=>{
      if (!cc) return cc;
      let log = [...cc.log];
      ...
      if (prospects.length < 6 && Math.random() < 0.5){
        const pName = randName();
        prospects.push({ name: pName, wins: 0, signed:false, retired:false });
        log.unshift(`🤝 Un jeune prometteur, ${pName}, rejoint ${cc.gymName}.`);
      }
      ...
      return { ...cc, season: cc.season+1, prospects, fightersTrained: prospects.length, championsProduced, ufcSignings, log };
    });
  }
  ...
  function startPromoterCareer(){
    setPromoterCareer({
      company: `${name} Promotions`, season: 1, revenue: 0, cardsRun: 0, bigNamesSigned: 0,
      log: [`🎪 ${name} raccroche les gants et fonde ${name} Promotions, bien décidé(e) à monter des galas mémorables.`],
    });
    setPhase("promoting");
  }
  function advancePromoterSeason(){
    setPromoterCareer(pc=>{
      if (!pc) return pc;
      let log = [...pc.log];
      let revenue = pc.revenue;
      let cardsRun = pc.cardsRun + 1;
      let bigNamesSigned = pc.bigNamesSigned;
      const gala = Math.round(rand(8000, 40000) * (1 + cardsRun*0.02));
      revenue += gala;
      log.unshift(`🎟️ Gala n°${cardsRun} organisé : ${eur(gala)} de recette nette.`);
      if (Math.random() < 0.3){
        const fighterName = randName();
        bigNamesSigned += 1;
        log.unshift(`✍️ ${pc.company} signe ${fighterName}, une tête d'affiche qui va faire vendre des billets.`);
      }
      if (Math.random() < 0.1){
        log.unshift(`⚠️ Un gala tourne au fiasco logistique — l'image de ${pc.company} en prend un coup.`);
        revenue -= Math.round(rand(2000,8000));
      }
      if (log.length > 40) log = log.slice(0,40);
      return { ...pc, season: pc.season+1, revenue, cardsRun, bigNamesSigned, log };
    });
  }
  ...
  function startCommentatorCareer(){
    setCommentatorCareer({
      season: 1, audience: 1000, viralMoments: 0, controversies: 0,
      log: [`🎙️ ${name} raccroche les gants et prend le micro pour commenter les grands cartels.`],
    });
    setPhase("commentating");
  }
  function advanceCommentatorSeason(){
    setCommentatorCareer(kc=>{
      if (!kc) return kc;
      let log = [...kc.log];
      let audience = kc.audience;
      let viralMoments = kc.viralMoments;
      let controversies = kc.controversies;
      const gain = Math.round(audience * rand(0.05, 0.22));
      audience += gain;
      log.unshift(`📈 Commentaire remarqué sur le dernier grand combat : +${gain} auditeurs (${audience} au total).`);
      if (Math.random() < 0.22){
        viralMoments += 1;
        const viralGain = Math.round(audience*rand(0.15,0.4));
        audience += viralGain;
        log.unshift(`🔥 Une punchline devient virale sur les réseaux — +${viralGain} auditeurs d'un coup !`);
      }
      if (Math.random() < 0.12){
        controversies += 1;
        log.unshift(`😬 Un commentaire maladroit fait polémique — une partie de l'audience se braque.`);
        audience = Math.round(audience*0.9);
      }
      if (log.length > 40) log = log.slice(0,40);
      return { ...kc, season: kc.season+1, audience, viralMoments, controversies, log };
    });
  }
  ...
  function applyLivingCosts(ns){
    ...
    const staffCost = Math.round((staffCostMonthly * (ownGym ? 0.75 : 1))/4.33);
  ...
  function pickFightAction(action){
    ...
    let oppInjuryRiskCageBonus = posInjuryAdj.oppAdj;
    {
      ...
      if (action.kind === "safe"){
        opt.cagePosScore = (opt.cagePosScore||0) * 0.6;
      } else {
        const successThisRound = roundDelta - posRoundBonus > 0;
  ...
  function resolveFight(opt, s){
    ...
    let severeInjury = Math.random() < injuryBase * 0.35 * (opt.riskyFailure ? 1.9 : 1);
    ...
    const embarrassing = (overallSkill(s) - opt.oppSkill > 15);
    ...
    if (win){
      ...
      if (!dominated && !embarrassing) statPenalty += rand(1,4);
  ...
  function overloadInjuryChance(load){
    const over = Math.max(0, (load||0) - 60) / 40;
    return Math.min(0.35, Math.pow(over, 2) * 0.32);
  }
    ...
    return (
      <div className="min-h-screen bg-zinc-950 text-zinc-100 flex items-center justify-center p-6">
        ...
        <div className="max-w-lg w-full">
          ...
          <div className="text-zinc-500 text-sm mb-4 text-center">Saison {cc.season}</div>
              ...
              <div className="space-y-1">
                {cc.prospects.map((p,i)=>(
                  <div key={i} className="flex justify-between text-zinc-300">
                    <span>{p.name} {p.signed && "✍️"} {p.retired && "🔚"}</span>
                    <span className="text-zinc-500">{p.wins} victoire{p.wins>1?"s":""}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <button onClick={advanceCoachingSeason}
            className={`${BTN} w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold uppercase tracking-wide py-3 rounded-xl mb-3`}>
            Faire avancer une saison
          </button>
          ...
          <button onClick={()=>{ setPhase("intro"); setName(""); setState(null); setLog([]); setHofSaved(false); setCoachCareer(null); }}
    ...
    return (
      <div className="min-h-screen bg-zinc-950 text-zinc-100 flex items-center justify-center p-6">
        ...
        <div className="max-w-lg w-full">
          ...
          <div className="text-zinc-500 text-sm mb-4 text-center">Saison {pc.season}</div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-4 grid grid-cols-3 gap-2 text-center">
            ...
            <div><div className="text-lg font-black text-emerald-400">{eur(pc.revenue)}</div><div className="text-[10px] text-zinc-500 uppercase">Recette cumulée</div></div>
          ...
          <button onClick={advancePromoterSeason}
            className={`${BTN} w-full bg-amber-600 hover:bg-amber-500 text-white font-bold uppercase tracking-wide py-3 rounded-xl mb-3`}>
            Organiser le prochain gala
          </button>
          ...
          <button onClick={()=>{ setPhase("intro"); setName(""); setState(null); setLog([]); setHofSaved(false); setPromoterCareer(null); }}
            className={`${BTN} w-full bg-red-600 hover:bg-red-500 text-white font-bold uppercase tracking-wide py-3 rounded-xl`}>
            Nouvelle carrière (combattant)
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-zinc-950 text-zinc-100 flex items-center justify-center p-6">
        ...
        <div className="max-w-lg w-full">
          ...
          <div className="text-zinc-500 text-sm mb-4 text-center">Saison {kc.season}</div>
          ...
          <button onClick={advanceCommentatorSeason}
            className={`${BTN} w-full bg-sky-600 hover:bg-sky-500 text-white font-bold uppercase tracking-wide py-3 rounded-xl mb-3`}>
            Commenter le prochain grand cartel
          </button>
          ...
          <button onClick={()=>{ setPhase("intro"); setName(""); setState(null); setLog([]); setHofSaved(false); setCommentatorCareer(null); }}
            className={`${BTN} w-full bg-red-600 hover:bg-red-500 text-white font-bold uppercase tracking-wide py-3 rounded-xl`}>
            Nouvelle carrière (combattant)
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-zinc-950 text-zinc-100 flex items-center justify-center p-6">
        ...
        <div className="max-w-lg w-full text-center">
          ...
          <button onClick={()=>{ setPhase("intro"); setName(""); setState(null); setLog([]); setHofSaved(false); }}
            className={`${BTN} w-full bg-red-600 hover:bg-red-500 text-white font-bold uppercase tracking-wide py-3 rounded-xl`}>
            Nouvelle carrière
          </button>
        </div>
